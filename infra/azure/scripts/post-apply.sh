...
echo
echo "5. VMs are for installing the agents (1 AKS only runs the controller):"
echo "   - Main: Podman/standalone agent path (bash vm-agent-example.sh <zone> <controller-ip>:9443)"
echo "   - For k8s+Cilium coexistence test (as previously mentioned): VMs also bootstrap k3s. After boot, ssh + /root/install-cilium.sh, copy k3s config, deploy agent DaemonSet into the VM's k3s (DFW protects host iface only)."
echo "6. Create Zone CRs for the zones we actually provisioned (see test_zones output for CIDRs)."
echo
